    ...
    this.gmapsApi.getNativeMap().then(map => {
      ...
      doNearbyQuery(servicePlaces, request).then((successMessage) => {
        ...
        directionsService.route(optRoute, function(response: any, status: any) {
          ...
          that.control++;
          if (status === 'OKAY') {
           console.log('routes are OKAY and ready to go and find the places');
            ...
            } else if (status == 'REQUEST is DENIED') {
            ...
            } else if (status == 'INVALID REQUEST AND REQUEST IS DENIED AND HAS NO PERMISSION') {
